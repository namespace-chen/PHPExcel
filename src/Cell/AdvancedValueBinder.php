...
/**
 * \PHPExcel\Cell\AdvancedValueBinder
 *
 ...
 */
class AdvancedValueBinder extends \PHPExcel\Cell\DefaultValueBinder implements \PHPExcel\Cell\IValueBinder
{
   /**
   ...
   public function bindValue(\PHPExcel\Cell $cell, $value = null)
   {
      ...
      // Style logic - strings
      if ($dataType === \PHPExcel\Cell\DataType::TYPE_STRING && !$value instanceof \PHPExcel\RichText) {
         //    Test for booleans using locale-setting
         if ($value == \PHPExcel\Calculation::getTRUE()) {
            $cell->setValueExplicit(true, \PHPExcel\Cell\DataType::TYPE_BOOL);
            return true;
         } elseif ($value == \PHPExcel\Calculation::getFALSE()) {
            $cell->setValueExplicit(false, \PHPExcel\Cell\DataType::TYPE_BOOL);
            return true;
         }

         // Check for number in scientific format
         if (preg_match('/^' . \PHPExcel\Calculation::CALCULATION_REGEXP_NUMBER . '$/', $value)) {
            $cell->setValueExplicit((float) $value, \PHPExcel\Cell\DataType::TYPE_NUMERIC);
            return true;
         }
         ...
         if (preg_match('/^\-?[0-9]*\.?[0-9]*\s?\%$/', $value)) {
            ...
            $cell->setValueExplicit($value, \PHPExcel\Cell\DataType::TYPE_NUMERIC);
            // Set style
            $cell->getWorksheet()->getStyle($cell->getCoordinate())
         ...
         if (preg_match('/^(\d|[0-1]\d|2[0-3]):[0-5]\d$/', $value)) {
            ...
            $days = $h / 24 + $m / 1440;
            $cell->setValueExplicit($days, \PHPExcel\Cell\DataType::TYPE_NUMERIC);
            // Set style
            $cell->getWorksheet()->getStyle($cell->getCoordinate())
         ...
         if (($d = \PHPExcel\Shared\Date::stringToExcel($value)) !== false) {
            // Convert value to number
            $cell->setValueExplicit($d, \PHPExcel\Cell\DataType::TYPE_NUMERIC);
