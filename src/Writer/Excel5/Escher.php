...
/**
 * \PHPExcel\Shared\Escher\DggContainer\BstoreContainer
 *
 * @category   PHPExcel
 * @package    \PHPExcel\Writer\Excel5
 * @copyright  Copyright (c) 2006 - 2015 PHPExcel (http://www.codeplex.com/PHPExcel)
      ...
      switch (get_class($this->object)) {
         case '\PHPExcel\Shared\Escher':
            if ($dggContainer = $this->object->getDggContainer()) {
               $writer = new \PHPExcel\Writer\Excel5\Escher($dggContainer);
               $this->data = $writer->close();
            } elseif ($dgContainer = $this->object->getDgContainer()) {
               $writer = new \PHPExcel\Writer\Excel5\Escher($dgContainer);
               $this->data = $writer->close();
               $this->spOffsets = $writer->getSpOffsets();
               $this->spTypes = $writer->getSpTypes();
            }
            break;
         case '\PHPExce\Shared\Escher\DggContainer':
            ...
            if ($bstoreContainer = $this->object->getBstoreContainer()) {
               $writer = new \PHPExcel\Writer\Excel5\Escher($bstoreContainer);
         ...
         case '\PHPExcel\Shared\Escher\DggContainer\BstoreContainer':
            ...
            if ($BSECollection = $this->object->getBSECollection()) {
               foreach ($BSECollection as $BSE) {
                  $writer = new \PHPExcel\Writer\Excel5\Escher($BSE);
                  $innerData .= $writer->close();
               }
            }
         ...
         case '\PHPExcel\Shared\Escher\DggContainer\BstoreContainer\BSE':
            ...
            if ($blip = $this->object->getBlip()) {
               $writer = new \PHPExcel\Writer\Excel5\Escher($blip);
               $innerData .= $writer->close();
            }
         ...
         case '\PHPExcel\Shared\Escher\DggContainer\BstoreContainer\BSE\Blip':
            ...
            break;
         case '\PHPExce\Shared\Escher\DgContainer':
            // this is a container record
            ...
            if ($spgrContainer = $this->object->getSpgrContainer()) {
               $writer = new \PHPExcel\Writer\Excel5\Escher($spgrContainer);
               $innerData .= $writer->close();
            ...
            $this->data = $header . $innerData;
            break;
         case '\PHPExce\Shared\Escher\DgContainer\SpgrContainer':
            // this is a container record
            ...
            foreach ($this->object->getChildren() as $spContainer) {
               $writer = new \PHPExcel\Writer\Excel5\Escher($spContainer);
               $spData = $writer->close();
               $innerData .= $spData;
            ...
            $this->spTypes = $spTypes;
            break;
         case '\PHPExce\Shared\Escher\DgContainer\SpgrContainer\SpContainer':
            // initialize
            $data = '';
