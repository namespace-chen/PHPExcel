 ...
 */
class HTML extends \PHPExcel\Writer\Abs implements \PHPExcel\Writer\IWriter
{
   /**
    * PHPExcel object
    *
   ...
   /**
    * Create a new \PHPExcel\Writer\HTML
    *
   ...
   private function extendRowsForChartsAndImages(\PHPExcel\Worksheet $pSheet, $row)
   {
      ...
      if ($this->includeCharts) {
         foreach ($pSheet->getChartCollection() as $chart) {
            if ($chart instanceof \PHPExcel\Chart) {
      ...
      foreach ($pSheet->getChartCollection() as $chart) {
         if ($chart instanceof \PHPExcel\Chart) {
            ...
            if ($chartCoordinates['cell'] == $coordinates) {
               $chartFileName = \PHPExcel\Shared\File::sys_get_temp_dir() . '/' . uniqid() . '.png';
   ...
   public function buildCSS($generateSurroundingHTML = true)
   {
      ...
      foreach ($sheets as $sheet) {
         ...
         if ($rowDimension->getRowHeight() == -1) {
            $pt_height = \PHPExcel\Shared\Font::getDefaultRowHeightByFont($this->phpExcel->getDefaultStyle()->getFont());
         } else {
         ...
         foreach ($pValues as $cellAddress) {
            ...
            if ($cell instanceof \PHPExcel\Cell) {
               ...
               if (!$this->useInlineCss) {
                  ...
                  if (
                     $sharedStyle->getAlignment()->getHorizontal() == \PHPExcel\Style\Alignment::HORIZONTAL_GENERAL
            ...
            if ($writeCell) {
               ...
               if (!$this->useInlineCss) {
                  ...
                  //** Necessary redundant code for the sake of \PHPExcel\Writer\PDF **
   ...
   private function setMargins(\PHPExcel\Worksheet $pSheet)
   {
      ...
      $left = \PHPExcel\Shared\String::FormatNumber($pSheet->getPageMargins()->getLeft()) . 'in; ';
      $htmlPage .= 'margin-left: ' . $left;
      $htmlBody .= 'margin-left: ' . $left;
      $right = \PHPExcel\Shared\String::FormatNumber($pSheet->getPageMargins()->getRight()) . 'in; ';
      $htmlPage .= 'margin-right: ' . $right;
      $htmlBody .= 'margin-right: ' . $right;
      $top = \PHPExcel\Shared\String::FormatNumber($pSheet->getPageMargins()->getTop()) . 'in; ';
      $htmlPage .= 'margin-top: ' . $top;
      $htmlBody .= 'margin-top: ' . $top;
      $bottom = \PHPExcel\Shared\String::FormatNumber($pSheet->getPageMargins()->getBottom()) . 'in; ';
