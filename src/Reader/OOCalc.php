 ...
 */
class OOCalc extends \PHPExcel\Reader\Abs implements \PHPExcel\Reader\IReader
{
   /**
    * Formats
    *
   ...
   /**
    * Create a new \PHPExcel\Reader\OOCalc
    */
   ...
   public function canRead($pFilename)
   {
      ...
      if ($zip->open($pFilename) === true) {
         ...
         if ($stat && ($stat['size'] <= 255)) {
            ...
            $xml = simplexml_load_string($this->securityScan($zip->getFromName('META-INF/manifest.xml')), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions());
   ...
   public function listWorksheetNames($pFilename)
   {
      ...
      if (!$zip->open($pFilename)) {
         throw new \PHPExcel\Reader\Exception("Could not open " . $pFilename . " for reading! Error opening file.");
      }

      $worksheetNames = array();

      $xml = new \XMLReader();
      $res = $xml->xml($this->securityScanFile('zip://' . realpath($pFilename) . '#content.xml'), null, \PHPExcel\Settings::getLibXmlLoaderOptions());
    ...
    *
    * @param     string         $pFilename
    * @param \PHPExcel\PHPExcel    $objPHPExcel
    * @return     PHPExcel
    * @throws     \PHPExcel\Reader\Exception
    */
   public function loadIntoExisting($pFilename, \PHPExcel\PHPExcel $objPHPExcel)
   {
      // Check if file exists
      if (!file_exists($pFilename)) {
         throw new \PHPExcel\Reader\Exception("Could not open " . $pFilename . " for reading! File does not exist.");
         ...
         foreach ($officePropertyMeta as $propertyName => $propertyValue) {
            ...
            switch ($propertyName) {
               ...
               case 'creation-date':
                  ...
                  break;
               case 'user-defined':
                  $propertyValueType = \PHPExcel\DocumentProperties::PROPERTY_TYPE_STRING;
                  foreach ($propertyValueAttributes as $key => $value) {
                     if ($key == 'name') {
                        ...
                        switch ($value) {
                           case 'date':
                              $propertyValue = \PHPExcel\DocumentProperties::convertProperty($propertyValue, 'date');
                              $propertyValueType = \PHPExcel\DocumentProperties::PROPERTY_TYPE_DATE;
                              break;
                           case 'boolean':
                              $propertyValue = \PHPExcel\DocumentProperties::convertProperty($propertyValue, 'bool');
                              $propertyValueType = \PHPExcel\DocumentProperties::PROPERTY_TYPE_BOOLEAN;
                              break;
                           case 'float':
                              $propertyValue = \PHPExcel\DocumentProperties::convertProperty($propertyValue, 'r4');
                              $propertyValueType = \PHPExcel\DocumentProperties::PROPERTY_TYPE_FLOAT;
                              break;
                           default:
                              $propertyValueType = \PHPExcel\DocumentProperties::PROPERTY_TYPE_STRING;
                        }
