 ...
 */
class Excel2007 extends \PHPExcel\Reader\Abs implements \PHPExcel\Reader\IReader
{
   /**
   ...
   /**
    * \PHPExcel\Reader\Excel2007\Theme instance
    *
    * @var \PHPExcel\Reader\Excel2007\Theme
    */
   private static $theme = null;

   /**
    * Create a new \PHPExcel\Reader\Excel2007 instance
    */
   ...
   public function canRead($pFilename)
   {
      ...
      //            throw new \PHPExcel\Reader\Exception($zipClass . " library is not enabled");
      //        }
   ...
   public function listWorksheetNames($pFilename)
   {
      ...
      $rels = simplexml_load_string(
         $this->securityScan($this->getFromZipArchive($zip, "_rels/.rels"), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions())
      ); //~ http://schemas.openxmlformats.org/package/2006/relationships");
      foreach ($rels->Relationship as $rel) {
         switch ($rel["Type"]) {
            case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
               $xmlWorkbook = simplexml_load_string(
                  $this->securityScan($this->getFromZipArchive($zip, "{$rel['Target']}"), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions())
   ...
   public function listWorksheetInfo($pFilename)
   {
      ...
      $rels = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "_rels/.rels")), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
      foreach ($rels->Relationship as $rel) {
         if ($rel["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument") {
            $dir = dirname($rel["Target"]);
            $relsWorkbook = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "$dir/_rels/" . basename($rel["Target"]) . ".rels")), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/package/2006/relationships");
            ...
            $xmlWorkbook = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "{$rel['Target']}")), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
            if ($xmlWorkbook->sheets) {
               $dir = dirname($rel["Target"]);
               foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                  $tmpInfo = array(
                  ...
                  $xml = new \XMLReader();
                  $res = $xml->xml($this->securityScanFile('zip://' . \PHPExcel\Shared\File::realpath($pFilename) . '#' . "$dir/$fileWorksheet"), null, \PHPExcel\Settings::getLibXmlLoaderOptions());
   ...
   public function load($pFilename)
   {
      ...
      $wbRels = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "xl/_rels/workbook.xml.rels")), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
         ...
         switch ($rel["Type"]) {
            case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme":
               ...
               if (is_object($xmlTheme)) {
                  ...
                  self::$theme = new \PHPExcel\Reader\Excel2007\Theme($themeName, $colourSchemeName, $themeColours);
               }
               break;
         }
      }
      ...
      foreach ($rels->Relationship as $rel) {
         switch ($rel["Type"]) {
               ...
               if (is_object($xmlCore)) {
                  ...
                  foreach ($xmlCore as $xmlProperty) {
                     ...
                     if (isset($cellDataOfficeAttributes['name'])) {
                        ...
                        $attributeValue = \PHPExcel\DocumentProperties::convertProperty($attributeValue, $attributeType);
                        $attributeType = \PHPExcel\DocumentProperties::convertPropertyType($attributeType);
            ...
            case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
               ...
               $xmlStrings = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "$dir/$xpath[Target]")), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
               ...
               if ($xmlWorkbook->sheets) {
                  foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                     ...
                     $fileWorksheet = $worksheets[(string) self::getArrayItem($eleSheet->attributes("http://schemas.openxmlformats.org/officeDocument/2006/relationships"), "id")];
                     $xmlSheet = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "$dir/$fileWorksheet")), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
                     ...
                     if (!$this->readDataOnly) {
                        ...
                        if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
                           $relsWorksheet = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
                     ...
                     if (!$this->readDataOnly) {
                        ...
                        if ($xmlSheet && $xmlSheet->legacyDrawingHF && !$this->readDataOnly) {
                           if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
                              ...
                              if ($vmlRelationship != '') {
                                 // Fetch linked images
                                 $relsVML = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, dirname($vmlRelationship) . '/_rels/' . basename($vmlRelationship) . '.rels')), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
                                 $drawings = array();
                                 ...
                                 foreach ($shapes as $idx => $shape) {
                                    ...
                                    $hfImages[(string)$shape['id']]->setPath("zip://" . \PHPExcel\Shared\File::realpath($pFilename) . "#" . $drawings[(string)$imageData['relid']], false);
                     ...
                     if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
                        ...
                        if ($xmlSheet->drawing && !$this->readDataOnly) {
                           foreach ($xmlSheet->drawing as $drawing) {
                              ...
                              $relsDrawing = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, dirname($fileDrawing) . "/_rels/" . basename($fileDrawing) . ".rels")), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions()); //~ http://schemas.openxmlformats.org/package/2006/relationships");
                              $images = array();
   ...
   private function readRibbon($excel, $customUITarget, $zip)
   {
      ...
      if ($dataRels) {
         ...
         $UIRels = simplexml_load_string($this->securityScan($dataRels), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions());
         if ($UIRels) {
