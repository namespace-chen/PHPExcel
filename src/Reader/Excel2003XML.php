 ...
 */
class Excel2003XML extends \PHPExcel\Reader\Abs implements \PHPExcel\Reader\IReader
{
   /**
    * Formats
    *
   ...
   /**
    * Create a new \PHPExcel\Reader\Excel2003XML
    */
   ...
   public function listWorksheetNames($pFilename)
   {
      ...
      if (!$this->canRead($pFilename)) {
         throw new \PHPExcel\Reader\Exception($pFilename . " is an Invalid Spreadsheet file.");
      }

      $worksheetNames = array();

      $xml = simplexml_load_string($this->securityScan(file_get_contents($pFilename)), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions());
   ...
   public function loadIntoExisting($pFilename, \PHPExcel\PHPExcel $objPHPExcel)
   {
      ...
      if (isset($xml->CustomDocumentProperties)) {
         foreach ($xml->CustomDocumentProperties[0] as $propertyName => $propertyValue) {
            ...
            $propertyType = \PHPExcel\DocumentProperties::PROPERTY_TYPE_UNKNOWN;
            switch ((string) $propertyAttributes) {
               case 'string':
                  $propertyType = \PHPExcel\DocumentProperties::PROPERTY_TYPE_STRING;
                  $propertyValue = trim($propertyValue);
                  break;
               case 'boolean':
                  $propertyType = \PHPExcel\DocumentProperties::PROPERTY_TYPE_BOOLEAN;
                  $propertyValue = (bool) $propertyValue;
                  break;
               case 'integer':
                  $propertyType = \PHPExcel\DocumentProperties::PROPERTY_TYPE_INTEGER;
                  $propertyValue = intval($propertyValue);
                  break;
               case 'float':
                  $propertyType = \PHPExcel\DocumentProperties::PROPERTY_TYPE_FLOAT;
                  $propertyValue = floatval($propertyValue);
                  break;
               case 'dateTime.tz':
                  $propertyType = \PHPExcel\DocumentProperties::PROPERTY_TYPE_DATE;
