...
class Excel5 extends \PHPExcel\Reader\Abs implements \PHPExcel\Reader\IReader
{
   // ParseXL definitions
    ...
    *
    * @var \PHPExcel\Reader\Excel5\RC4
    */
   private $rc4Key = null;
   ...
   /**
    * Create a new \PHPExcel\Reader\Excel5 instance
    */
   public function __construct()
   {
      $this->readFilter = new \PHPExcel\Reader\DefaultReadFilter();
   }
   ...
   public function canRead($pFilename)
   {
      ...
      if (!file_exists($pFilename)) {
         throw new \PHPExcel\Reader\Exception("Could not open " . $pFilename . " for reading! File does not exist.");
      }
   ...
   public function load($pFilename)
   {
      ...
      // Initialisations
      $this->phpExcel = new \PHPExcel\PHPExcel;
      ...
      if (!$this->readDataOnly) {
         foreach ($this->objFonts as $objFont) {
            if (isset($objFont->colorIndex)) {
               $color = \PHPExcel\Reader\Excel5\Color::map($objFont->colorIndex, $this->palette, $this->version);
         ...
         foreach ($this->phpExcel->getCellXfCollection() as $objStyle) {
            ...
            if (isset($fill->startcolorIndex)) {
               $startColor = \PHPExcel\Reader\Excel5\Color::map($fill->startcolorIndex, $this->palette, $this->version);
               $fill->getStartColor()->setRGB($startColor['rgb']);
            }
            if (isset($fill->endcolorIndex)) {
               $endColor = \PHPExcel\Reader\Excel5\Color::map($fill->endcolorIndex, $this->palette, $this->version);
            ...
            if (isset($top->colorIndex)) {
               $borderTopColor = \PHPExcel\Reader\Excel5\Color::map($top->colorIndex, $this->palette, $this->version);
               $top->getColor()->setRGB($borderTopColor['rgb']);
            }
            if (isset($right->colorIndex)) {
               $borderRightColor = \PHPExcel\Reader\Excel5\Color::map($right->colorIndex, $this->palette, $this->version);
               $right->getColor()->setRGB($borderRightColor['rgb']);
            }
            if (isset($bottom->colorIndex)) {
               $borderBottomColor = \PHPExcel\Reader\Excel5\Color::map($bottom->colorIndex, $this->palette, $this->version);
               $bottom->getColor()->setRGB($borderBottomColor['rgb']);
            }
            if (isset($left->colorIndex)) {
               $borderLeftColor = \PHPExcel\Reader\Excel5\Color::map($left->colorIndex, $this->palette, $this->version);
               $left->getColor()->setRGB($borderLeftColor['rgb']);
            }
            if (isset($diagonal->colorIndex)) {
               $borderDiagonalColor = \PHPExcel\Reader\Excel5\Color::map($diagonal->colorIndex, $this->palette, $this->version);
      ...
      if (!$this->readDataOnly && $this->drawingGroupData) {
         ...
         $reader = new \PHPExcel\Reader\Excel5\Escher($escherWorkbook);
         $escherWorkbook = $reader->load($this->drawingGroupData);

         // debug Escher stream
         //$debug = new Debug_Escher(new \PHPExcel\Shared\Escher());
      ...
      foreach ($this->sheets as $sheet) {
         ...
         if (!$this->readDataOnly && $this->drawingData) {
            ...
            $reader = new \PHPExcel\Reader\Excel5\Escher($escherWorksheet);
   ...
   private function readRecordData($data, $pos, $len)
   {
      ...
      if ($this->encryption == self::MS_BIFF_CRYPTO_RC4) {
         ...
         throw new \PHPExcel\Reader\Exception('XOr encryption not supported');
      }
      return $recordData;
   }
      ...
      switch ($substreamType) {
         case self::XLS_WorkbookGlobals:
            ...
            if (($version != self::XLS_BIFF8) && ($version != self::XLS_BIFF7)) {
               throw new \PHPExcel\Reader\Exception('Cannot read this Excel file. Version is too old.');
            }
   ...
   private function readFilepass()
   {
      ...
      if ($length != 54) {
         throw new \PHPExcel\Reader\Exception('Unexpected file pass record length');
      }
      ...
      if (!$this->verifyPassword('VelvetSweatshop', substr($recordData, 6, 16), substr($recordData, 22, 16), substr($recordData, 38, 16), $this->md5Ctxt)) {
         throw new \PHPExcel\Reader\Exception('Decryption password incorrect');
      }
    ...
    *
    * @return \PHPExcel\Reader\Excel5\RC4
    */
   private function makeKey($block, $valContext)
   {
      ...
      $md5 = new \PHPExcel\Reader\Excel5\MD5();
      $md5->add($pwarray);

      $s = $md5->getContext();
      return new \PHPExcel\Reader\Excel5\RC4($s);
   }
   ...
   private function readXf()
   {
      ...
      if (!$this->readDataOnly) {
         ...
         if ($this->version == self::XLS_BIFF8) {
            ...
            if ($bordersLeftStyle = \PHPExcel\Reader\Excel5\Style\Border::lookup((0x0000000F & self::getInt4d($recordData, 10)) >> 0)) {
               $objStyle->getBorders()->getLeft()->setBorderStyle($bordersLeftStyle);
            }
            // bit: 7-4; mask: 0x000000F0; right style
            if ($bordersRightStyle = \PHPExcel\Reader\Excel5\Style\Border::lookup((0x000000F0 & self::getInt4d($recordData, 10)) >> 4)) {
               $objStyle->getBorders()->getRight()->setBorderStyle($bordersRightStyle);
            }
            // bit: 11-8; mask: 0x00000F00; top style
            if ($bordersTopStyle = \PHPExcel\Reader\Excel5\Style\Border::lookup((0x00000F00 & self::getInt4d($recordData, 10)) >> 8)) {
               $objStyle->getBorders()->getTop()->setBorderStyle($bordersTopStyle);
            }
            // bit: 15-12; mask: 0x0000F000; bottom style
            if ($bordersBottomStyle = \PHPExcel\Reader\Excel5\Style\Border::lookup((0x0000F000 & self::getInt4d($recordData, 10)) >> 12)) {
            ...
            if ($bordersDiagonalStyle = \PHPExcel\Reader\Excel5\Style\Border::lookup((0x01E00000 & self::getInt4d($recordData, 14)) >> 21)) {
               $objStyle->getBorders()->getDiagonal()->setBorderStyle($bordersDiagonalStyle);
            }

            // bit: 31-26; mask: 0xFC000000 fill pattern
            if ($fillType = \PHPExcel\Reader\Excel5\Style\FillPattern::lookup((0xFC000000 & self::getInt4d($recordData, 14)) >> 26)) {
            ...
            $objStyle->getFill()->setFillType(\PHPExcel\Reader\Excel5\Style\FillPattern::lookup((0x003F0000 & $borderAndBackground) >> 16));

            // bit: 24-22; mask: 0x01C00000; bottom line style
            $objStyle->getBorders()->getBottom()->setBorderStyle(\PHPExcel\Reader\Excel5\Style\Border::lookup((0x01C00000 & $borderAndBackground) >> 22));
            ...
            $objStyle->getBorders()->getTop()->setBorderStyle(\PHPExcel\Reader\Excel5\Style\Border::lookup((0x00000007 & $borderLines) >> 0));

            // bit: 5-3; mask: 0x00000038; left line style
            $objStyle->getBorders()->getLeft()->setBorderStyle(\PHPExcel\Reader\Excel5\Style\Border::lookup((0x00000038 & $borderLines) >> 3));

            // bit: 8-6; mask: 0x000001C0; right line style
            $objStyle->getBorders()->getRight()->setBorderStyle(\PHPExcel\Reader\Excel5\Style\Border::lookup((0x000001C0 & $borderLines) >> 6));
   ...
   private function readFormula()
   {
      ...
      if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
         ...
         if ((ord($recordData{
            ...
            $value = \PHPExcel\Reader\Excel5\ErrorCode::lookup(ord($recordData{
               8}));
         } elseif ((ord($recordData{
               6}) == 3)
            && (ord($recordData{
         ...
         if (!$isPartOfSharedFormula) {
            ...
            try {
               if ($this->version != self::XLS_BIFF8) {
                  throw new \PHPExcel\Reader\Exception('Not BIFF8. Can only read BIFF8 formulas');
               }
      ...
      if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
         ...
         switch ($isError) {
            case 0: // boolean
               ...
               break;
            case 1: // error type
               $value = \PHPExcel\Reader\Excel5\ErrorCode::lookup($boolErr);
      ...
      if (!$this->readDataOnly) {
         ...
         switch ($sz) {
            case 0x14:
               ...
               $color = \PHPExcel\Reader\Excel5\Color::map($colorIndex, $this->palette, $this->version);
   ...
   private function getNextToken($formulaData, $baseCell = 'A1')
   {
      ...
      switch ($id) {
               ...
               case 0x41:
                  ...
                  switch (ord($formulaData[2])) {
                     ...
                     case 0x05:
                        ...
                        break;
                     default:
                        throw new \PHPExcel\Reader\Exception('Unrecognized space type in tAttrSpace token');
                        break;
                  }
                  ...
                  break;
               default:
                  throw new \PHPExcel\Reader\Exception('Unrecognized attribute flag in tAttr token');
                  break;
            }
            break;
         case 0x1C:    //    error code
            ...
            $name = 'tErr';
            $size = 2;
            $data = \PHPExcel\Reader\Excel5\ErrorCode::lookup(ord($formulaData[1]));
            break;
         case 0x1D:    //    boolean
         ...
         case 0x61:
            ...
            switch (self::getInt2d($formulaData, 1)) {
               ...
               case 368:
                  ...
                  break;
               default:
                  throw new \PHPExcel\Reader\Exception('Unrecognized function in formula');
                  break;
            }
         ...
         default:
            throw new \PHPExcel\Reader\Exception('Unrecognized token ' . sprintf('%02X', $id) . ' in formula');
            break;
      }
   ...
   private function readBIFF5CellRangeAddressFixed($subData)
   {
      ...
      // check values
      if ($fr > $lr || $fc > $lc) {
         throw new \PHPExcel\Reader\Exception('Not a cell range address');
      }
    ...
    * in which case an \PHPExcel\Reader\Exception is thrown
    *
    * @param int $index
    * @return string|false
    * @throws \PHPExcel\Reader\Exception
         ...
         switch ($type) {
            case 'internal':
               ...
               if ($this->ref[$index]['firstSheetIndex'] == 0xFFFF or $this->ref[$index]['lastSheetIndex'] == 0xFFFF) {
                  throw new \PHPExcel\Reader\Exception('Deleted sheet reference');
               }
            ...
            default:
               ...
               throw new \PHPExcel\Reader\Exception('Excel5 reader only supports internal sheets in fomulas');
               break;
         }
      }
      return false;
   ...
   private static function readBIFF8Constant($valueData)
   {
      ...
      switch ($identifier) {
         ...
         case 0x10: // error code
            // offset: 1; size: 1; error code
            $value = \PHPExcel\Reader\Excel5\ErrorCode::lookup(ord($valueData[1]));
            $size = 9;
            break;
      }
      return array(
