...
class LookupRef
{
   ...
   {
      $row        = \PHPExcel\Calculation\Funcs::flattenSingleValue($row);
      $column     = \PHPExcel\Calculation\Funcs::flattenSingleValue($column);
      $relativity = \PHPExcel\Calculation\Funcs::flattenSingleValue($relativity);
      $sheetText  = \PHPExcel\Calculation\Funcs::flattenSingleValue($sheetText);
   ...
   public static function COLUMN($cellAddress = null)
   {
      ...
      if (is_array($cellAddress)) {
         foreach ($cellAddress as $columnKey => $value) {
            ...
            return (int) \PHPExcel\Cell::columnIndexFromString($columnKey);
         }
      } else {
         ...
         if (strpos($cellAddress, ':') !== false) {
            ...
            $returnValue = array();
            do {
               $returnValue[] = (int) \PHPExcel\Cell::columnIndexFromString($startAddress);
            } while ($startAddress++ != $endAddress);
            return $returnValue;
         } else {
            $cellAddress = preg_replace('/[^a-z]/i', '', $cellAddress);
            return (int) \PHPExcel\Cell::columnIndexFromString($cellAddress);
         }
      }
   }
   ...
   public static function HYPERLINK($linkURL = '', $displayName = null, \PHPExcel\Cell $pCell = null)
   {
      ...
      $linkURL     = (is_null($linkURL))     ? '' : \PHPExcel\Calculation\Funcs::flattenSingleValue($linkURL);
      $displayName = (is_null($displayName)) ? '' : \PHPExcel\Calculation\Funcs::flattenSingleValue($displayName);

      if ((!is_object($pCell)) || (trim($linkURL) == '')) {
         return \PHPExcel\Calculation\Funcs::REF();
      }
   ...
   {
      $cellAddress    = \PHPExcel\Calculation\Funcs::flattenSingleValue($cellAddress);
      if (is_null($cellAddress) || $cellAddress === '') {
         return \PHPExcel\Calculation\Funcs::REF();
      }
   ...
   {
      $rows    = \PHPExcel\Calculation\Funcs::flattenSingleValue($rows);
      $columns = \PHPExcel\Calculation\Funcs::flattenSingleValue($columns);
      $height  = \PHPExcel\Calculation\Funcs::flattenSingleValue($height);
      $width   = \PHPExcel\Calculation\Funcs::flattenSingleValue($width);
      if ($cellAddress == null) {
         return 0;
      }
   ...
   public static function CHOOSE()
   {
      ...
      $chosenEntry = \PHPExcel\Calculation\Funcs::flattenArray(array_shift($chooseArgs));
      ...
      if (is_array($chooseArgs[$chosenEntry])) {
         return \PHPExcel\Calculation\Funcs::flattenArray($chooseArgs[$chosenEntry]);
      } else {
   ...
   {
      $lookup_array = \PHPExcel\Calculation\Funcs::flattenArray($lookup_array);
      $lookup_value = \PHPExcel\Calculation\Funcs::flattenSingleValue($lookup_value);
      $match_type    = (is_null($match_type)) ? 1 : (int) \PHPExcel\Calculation\Funcs::flattenSingleValue($match_type);
   ...
   {
      $lookup_value    = \PHPExcel\Calculation\Funcs::flattenSingleValue($lookup_value);
      $index_number    = \PHPExcel\Calculation\Funcs::flattenSingleValue($index_number);
      $not_exact_match = \PHPExcel\Calculation\Funcs::flattenSingleValue($not_exact_match);
