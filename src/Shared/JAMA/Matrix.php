...
class Matrix
{
      ...
      if (func_num_args() > 0) {
         ...
         switch ($match) {
            ...
            case 'integer,integer':
               ...
               $R = new \PHPExcel\Shared\JAMA\Matrix($m, $n);
               for ($i = $i0; $i < $this->m; ++$i) {
                  for ($j = $j0; $j < $this->n; ++$j) {
            ...
            case 'integer,integer,integer,integer':
               ...
               $R = new \PHPExcel\Shared\JAMA\Matrix($m + 1, $n + 1);
               for ($i = $i0; $i <= $iF; ++$i) {
                  for ($j = $j0; $j <= $jF; ++$j) {
            ...
            case 'array,integer,integer':
               ...
               $R = new \PHPExcel\Shared\JAMA\Matrix($m, $n + 1);
               for ($i = 0; $i < $m; ++$i) {
                  for ($j = $j0; $j <= $jF; ++$j) {
   ...
   {
      if ($B instanceof \PHPExcel\Shared\JAMA\Matrix) {
   ...
   {
      $R = new \PHPExcel\Shared\JAMA\Matrix($this->n, $this->m);
      for ($i = 0; $i < $this->m; ++$i) {
         for ($j = 0; $j < $this->n; ++$j) {
   ...
   public function plus()
   {
      if (func_num_args() > 0) {
         ...
         switch ($match) {
            case 'object':
               if ($args[0] instanceof \PHPExcel\Shared\JAMA\Matrix) {
                  $M = $args[0];
               } else {
                  throw new \PHPExcel\Calculation\Exception(self::ARGUMENT_TYPE_EXCEPTION);
               }
               break;
            case 'array':
               $M = new \PHPExcel\Shared\JAMA\Matrix($args[0]);
               break;
            default:
               throw new \PHPExcel\Calculation\Exception(self::POLYMORPHIC_ARGUMENT_EXCEPTION);
               break;
         ...
         for ($i = 0; $i < $this->m; ++$i) {
            for ($j = 0; $j < $this->n; ++$j) {
               ...
               if ($validValues) {
                  ...
                  $this->A[$i][$j] = \PHPExcel\Calculation\Funcs::NaN();
               }
            }
         }
         return $this;
         ...
         for ($i = 0; $i < $this->m; ++$i) {
            for ($j = 0; $j < $this->n; ++$j) {
               ...
               if ($validValues) {
                  ...
                  $M->set($i, $j, \PHPExcel\Calculation\Funcs::NaN());
               }
            }
         }
         return $M;
      ...
      if (func_num_args() > 0) {
         ...
         switch ($match) {
            case 'object':
               ...
               if ($this->n == $B->m) {
                  $C = new \PHPExcel\Shared\JAMA\Matrix($this->m, $B->n);
                  for ($j = 0; $j < $B->n; ++$j) {
                     for ($k = 0; $k < $this->n; ++$k) {
                        $Bcolj[$k] = $B->A[$k][$j];
                     }
               ...
               break;
            case 'array':
               $B = new \PHPExcel\Shared\JAMA\Matrix($args[0]);
               if ($this->n == $B->m) {
                  $C = new \PHPExcel\Shared\JAMA\Matrix($this->m, $B->n);
                  for ($i = 0; $i < $C->m; ++$i) {
                     for ($j = 0; $j < $C->n; ++$j) {
               ...
               break;
            case 'integer':
               $C = new \PHPExcel\Shared\JAMA\Matrix($this->A);
               for ($i = 0; $i < $C->m; ++$i) {
                  for ($j = 0; $j < $C->n; ++$j) {
                     $C->A[$i][$j] *= $args[0];
                  }
               ...
               break;
            case 'double':
               $C = new \PHPExcel\Shared\JAMA\Matrix($this->m, $this->n);
               for ($i = 0; $i < $C->m; ++$i) {
                  for ($j = 0; $j < $C->n; ++$j) {
   ...
   {
      if ($this->m == $this->n) {
         $LU = new \PHPExcel\Shared\JAMA\LUDecomposition($this);
         return $LU->solve($B);
      } else {
         $QR = new \PHPExcel\Shared\JAMA\QRDecomposition($this);
         return $QR->solve($B);
      }
   }
   ...
   public function det()
   {
      $L = new \PHPExcel\Shared\JAMA\LUDecomposition($this);
      return $L->det();
   }
}
