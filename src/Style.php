 ...
 */
class Style extends \PHPExcel\Style\Supervisor implements \PHPExcel\IComparable
{
   /**
    * Font
    *
    ...
    * Number Format
    *
    * @var \PHPExcel\Style\NumberFormat
    */
    ...
    * Conditional styles
    *
    * @var \PHPExcel\Style\Conditional[]
    */
   ...
   public function __construct($isSupervisor = false, $isConditional = false)
   {
      ...
      $this->font         = new \PHPExcel\Style\Font($isSupervisor, $isConditional);
      $this->fill         = new \PHPExcel\Style\Fill($isSupervisor, $isConditional);
      $this->borders      = new \PHPExcel\Style\Borders($isSupervisor, $isConditional);
      $this->alignment    = new \PHPExcel\Style\Alignment($isSupervisor, $isConditional);
      $this->numberFormat = new \PHPExcel\Style\NumberFormat($isSupervisor, $isConditional);
      $this->protection   = new \PHPExcel\Style\Protection($isSupervisor, $isConditional);
   ...
   public function applyFromArray($pStyles = null, $pAdvanced = true)
   {
      if (is_array($pStyles)) {
         if ($this->isSupervisor) {
            ...
            $rangeStart = \PHPExcel\Cell::coordinateFromString($rangeA);
            $rangeEnd   = \PHPExcel\Cell::coordinateFromString($rangeB);

            // Translate column into index
            $rangeStart[0]    = \PHPExcel\Cell::columnIndexFromString($rangeStart[0]) - 1;
            $rangeEnd[0]    = \PHPExcel\Cell::columnIndexFromString($rangeEnd[0]) - 1;
            ...
            if ($pAdvanced && isset($pStyles['borders'])) {
               ...
               for ($x = 1; $x <= $xMax; ++$x) {
                  ...
                  $colStart = ($x == 3) ?
                     \PHPExcel\Cell::stringFromColumnIndex($rangeEnd[0])
                     : \PHPExcel\Cell::stringFromColumnIndex($rangeStart[0] + $x - 1);
                  // end column index for region
                  $colEnd = ($x == 1) ?
                     \PHPExcel\Cell::stringFromColumnIndex($rangeStart[0])
                     : \PHPExcel\Cell::stringFromColumnIndex($rangeEnd[0] - $xMax + $x);
    ...
    * @return \PHPExcel\Style
    */
   public function setFont(\PHPExcel\Style\Font $font)
   {
      $this->font = $font;
      return $this;
   }
    ...
    * Get Alignment
    *
    * @return \PHPExcel\Style\Alignment
    */
    ...
    * Get Number Format
    *
    * @return \PHPExcel\Style\NumberFormat
    */
    ...
    *
    * @param \PHPExcel\Style\Conditional[] $pValue Array of condtional styles
    * @return \PHPExcel\Style
    */
    ...
    * Get Protection
    *
    * @return \PHPExcel\Style\Protection
    */
