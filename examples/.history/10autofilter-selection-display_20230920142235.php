...
$objXLSX->getActiveSheet()->setCellValue('A1', 'Financial Year')
	...
	foreach($periods as $period) {
		foreach($countries as $country) {
			...
			for($i = 1; $i <= $endDays; ++$i) {
				...
				$objXLSX->getActiveSheet()->fromArray($dataArray, NULL, 'A'.$row++);
			}
		}
	}
}
...
$objXLSX->getActiveSheet()->getStyle('A1:F1')->getFont()->setBold(true);
$objXLSX->getActiveSheet()->getStyle('A1:F1')->getAlignment()->setWrapText(TRUE);
$objXLSX->getActiveSheet()->getColumnDimension('C')->setWidth(12.5);
$objXLSX->getActiveSheet()->getColumnDimension('D')->setWidth(10.5);
$objXLSX->getActiveSheet()->getStyle('D2:D'.$row)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_DATE_YYYYMMDD2);
$objXLSX->getActiveSheet()->getStyle('E2:F'.$row)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_USD_SIMPLE);
$objXLSX->getActiveSheet()->getColumnDimension('F')->setWidth(14);
$objXLSX->getActiveSheet()->freezePane('A2');
...
$objXLSX->getActiveSheet()->setAutoFilter($objXLSX->getActiveSheet()->calculateWorksheetDimension());

// Set active filters
$autoFilter = $objXLSX->getActiveSheet()->getAutoFilter();
...
foreach ($objXLSX->getActiveSheet()->getRowIterator() as $row) {
	if ($objXLSX->getActiveSheet()->getRowDimension($row->getRowIndex())->getVisible()) {
		echo '    Row number - ' , $row->getRowIndex() , ' ';
		echo $objXLSX->getActiveSheet()->getCell('C'.$row->getRowIndex())->getValue(), ' ';
		echo $objXLSX->getActiveSheet()->getCell('D'.$row->getRowIndex())->getFormattedValue(), ' ';
		echo EOL;
	}
}
