...
$objXLSX->getActiveSheet()->setCellValue('C22', '=COUNTA(C2:C12)')
                              ->setCellValue('C23', '=MAXA(C2:C12)')
                              ->setCellValue('C24', '=MINA(C2:C12)');

$objXLSX->getActiveSheet()->setCellValue('C26', '=STDEV(C2:C12)')
                              ->setCellValue('C27', '=STDEVA(C2:C12)')
                              ->setCellValue('C28', '=STDEVP(C2:C12)')
                              ->setCellValue('C29', '=STDEVPA(C2:C12)');

$objXLSX->getActiveSheet()->setCellValue('C31', '=DEVSQ(C2:C12)')
...
$objXLSX->getActiveSheet()->setCellValue('D14', '=((D2 * D3) + D4) & " should be 10"');

$objXLSX->getActiveSheet()->setCellValue('E12', 'Other functions')
                              ->setCellValue('E14', '=PI()')
                              ->setCellValue('E15', '=RAND()')
                              ->setCellValue('E16', '=RANDBETWEEN(5, 10)');

$objXLSX->getActiveSheet()->setCellValue('E17', 'Count of both ranges:')
                              ->setCellValue('F17', '=COUNT(B2:C12)');

$objXLSX->getActiveSheet()->setCellValue('E18', 'Total of both ranges:')
                              ->setCellValue('F18', '=SUM(B2:C12)');

$objXLSX->getActiveSheet()->setCellValue('E19', 'Maximum of both ranges:')
                              ->setCellValue('F19', '=MAX(B2:C12)');

$objXLSX->getActiveSheet()->setCellValue('E20', 'Minimum of both ranges:')
                              ->setCellValue('F20', '=MIN(B2:C12)');

$objXLSX->getActiveSheet()->setCellValue('E21', 'Average of both ranges:')
                              ->setCellValue('F21', '=AVERAGE(B2:C12)');

$objXLSX->getActiveSheet()->setCellValue('E22', 'Median of both ranges:')
                              ->setCellValue('F22', '=MEDIAN(B2:C12)');

$objXLSX->getActiveSheet()->setCellValue('E23', 'Mode of both ranges:')
...
for ($col = 'B'; $col != 'G'; ++$col) {
    for($row = 14; $row <= 41; ++$row) {
        if ((!is_null($formula = $objXLSX->getActiveSheet()->getCell($col.$row)->getValue())) &&
			($formula[0] == '=')) {
            echo 'Value of ' , $col , $row , ' [' , $formula , ']: ' ,
                               $objXLSX->getActiveSheet()->getCell($col.$row)->getCalculatedValue() . EOL;
        }
    }
}
