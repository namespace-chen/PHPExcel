...
foreach(glob('./data/continents/*') as $key => $filename) {
    ...
    $objXLSX->addNamedRange(
        new PHPExcel_NamedRange(
            $continent, 
            $objXLSX->getActiveSheet(), $column . '1:' . $column . $countryCount
        )
    );
    $objXLSX->getActiveSheet()
...
$objXLSX->addNamedRange(
    new PHPExcel_NamedRange(
        'Continents', 
        $objXLSX->getActiveSheet(), $continentColumn . '1:' . $continentColumn . ($key+1)
    )
);
...
$objValidation = $objXLSX->getActiveSheet()
    ->getCell('B1')
    ->getDataValidation();
...
$objXLSX->getActiveSheet()->getColumnDimension('A')->setWidth(12);
$objXLSX->getActiveSheet()->getColumnDimension('B')->setWidth(30);
